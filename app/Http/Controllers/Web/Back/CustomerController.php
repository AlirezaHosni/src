...
class CustomerController extends Controller
{
    ...
    {
        $user = User::where(['is_admin' => '0', 'is_seller' => 0])->orderBy('id', 'desc')->with('profiles', 'address.province','address.city')->get();
        return view('admin.customer.all', compact('user'));
    }

    public function search(Request $request)
    {
        if (strlen($request->customer_phone) == 4){
            $user = User::where('phone', 'like', "%$request->customer_phone")->where(['is_admin' => '0', 'is_seller' => 0])->orderBy('id', 'desc')->with('profiles', 'address.province','address.city')->get();
        }else
            $user = User::where('phone', 'like', "%$request->customer_phone%")->where(['is_admin' => '0', 'is_seller' => 0])->orderBy('id', 'desc')->with('profiles', 'address.province','address.city')->get();
