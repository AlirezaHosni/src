...
class ArticleController extends Controller
{
    ...
    public function searchNews(Request $request)
    {
        ...
        if(Auth::check()){
            if (auth()->user()->is_admin == 1 or auth()->user()->is_seller == 1)
                $searchResults = News::where('title','like', "%$newsTitle%")->latest()->get();
            else
                $searchResults = News::where('news_type','public')->where('title','like', "%$newsTitle%")->latest()->get();
        }else{
            $searchResults = News::where('news_type','public')->where('title','like', "%$newsTitle%")->latest()->get();
            //$news = News::where('news_type','public')->latest()->take(9)->get();
        }
        return view('front.articles.news_all',compact('searchResults'));
    }
}
