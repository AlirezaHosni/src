...
class OrdersController extends Controller
{
    ...
    public function referral(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            //dd($data);
            $parentOrder = Order::where('id', $data['order_id'])->first();
            if (Carbon::parse($parentOrder->created_at)->addDays(7) < Carbon::now())
                return redirect()->back()->with('flash_message_error', 'زمان مرجوعی کالا گذشته است');

            $order = OrderProduct::where('order_id','=',$data['order_id'])->first();
            $order->referral_text = $data['referral_text'];
            $order->sefaresh_code = $data['sefaresh_code'];
            $order->save();
            return redirect()->route('sellers.referalShow');
        }
        $user_id = Auth::user()->id;
//->addDay(10)
    ...
    {
        $user_id = Auth::user()->id;
        $status = Order::where('tracking_code', $request->tracking_code)->where('user_id', $user_id)->get();
